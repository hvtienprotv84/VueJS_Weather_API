<template>
  ...
  <div id="app" class="" :class="darkmode == true ? 'dark':''" >
    <main>
      <div class="search-box w-[700px] m-auto">
        ...
        <a href="/" class="modebtn hover:bg-black hover:text-white dark:text-white dark:border-white dark:hover:bg-white dark:hover:text-black absolute">
          Refresh
        </a>
      </div>
      <br>
      <div class="weather-wrap text-center" v-if="typeof weather.main != 'undefined'">
        ...
        <br>
        <button v-on:click="compare_places" class="mt-[10px] font-bold dark:text-white">Ghi Nhận Khu Vực</button>
        <div v-for="(weatherlists, index) in weatherlist" :key="weatherlists" class="dark:text-white">
        {{index + 1}} - {{weatherlists.name}}
      </div>
...
export default {
  name: 'app',
  mounted() {  
    document.title = 'Tiến - VueJS - Weather API';  
  }, 
  data() {
    return {
